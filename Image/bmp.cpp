...
bool BMPReadImage( FILE *fp , Image32& img )
{
	...
	* issues.  Sigh... */
	...
	/* Check file header */
	if( bmfh.bfType!=BMP_BF_TYPE ) return false;
	/* ignore bmfh.bfSize */
	/* ignore bmfh.bfReserved1 */
	/* ignore bmfh.bfReserved2 */
	if( bmfh.bfOffBits!=BMP_BF_OFF_BITS ) return false;
	...
	/* Check info header */
	if( bmih.biSize!=BMP_BI_SIZE ) return false;
	if( bmih.biWidth<=0 ) return false;
	if( bmih.biHeight<=0 ) return false;
	if( bmih.biPlanes!=1 ) return false;
	if( bmih.biBitCount!=24 ) return false;
	if( bmih.biCompression!=BI_RGB ) return false;
	lineLength = bmih.biWidth * 3;	/* RGB */
	if( (lineLength % 4)!=0 ) lineLength = (lineLength / 4 + 1) * 4;
	if( bmih.biSizeImage!=(DWORD) lineLength * (DWORD) bmih.biHeight ) return false;
